...
int http_GET_handler(struct soap *soap)
{
  ...
  if (check_authentication(soap))
      return http_fget_error(soap, "HTTP Error", "401 Unauthorized", "", 401); /* HTTP not authorized */
...
/******************************************************************************\
 *
 *      Dynamic HTTP GET application/x-www-form-urlencoded 
 *
\******************************************************************************/

int wfs2get(struct soap *soap)
{
    /* parse argument of query form a dynamic HTTP GET application/x-www-form-urlencoded request */
    string service, version, acceptversion, req, srs, typenames, bbox, inputid, storedq;
    stringstream e;
    ...
    while (s)
    {
        ...
        if (key && val)
        {
            ...
            if (!strcmp(key, "service")){
                ...
                version.assign(val);
            else if (!strcmp(key, "acceptversions"))
                acceptversion.assign(val);
                ...
                inputid.assign(val);
            else if (!strcmp(key, "srsname")){
                boost::to_lower(val);
                srs.assign(val);
            }
            else if (!strcmp(key, "typenames")){
                boost::to_lower(val);
                typenames.assign(val);
            }
            else if (!strcmp(key, "bbox")){
                boost::to_lower(val);
                bbox.assign(val);
            }
            else if (!strcmp(key, "storedquery_id")){
                boost::to_lower(val);
                storedq.assign(val);
            }
        }
    ...
    if (service.empty()){
        //The requested service name is not provided
        e << "This expected parameter value is missing in the request";
        return http_fget_error(soap, "MissingParameterValue", e.str(), "service", 400);
    }
    if (service.compare("wfs")){
        //The requested service is not implemented
        e << "The requested service is unknown or not implemented by this server";
        return http_fget_error(soap,"OperationNotSupported", e.str(), service, 501);
    }
    if(req.empty()){
        //The requested operation name is not provided
        e << "This expected parameter value is missing in the request";
        return http_fget_error(soap, "MissingParameterValue", e.str(), "request", 400);
    }
    if (req.compare("getcapabilities") && version.empty()){
        //The requested version number is not provided
        e << "This expected parameter value is missing in the request";
        return http_fget_error(soap, "MissingParameterValue", e.str(), "version", 400);
    }
    if (!req.compare("getcapabilities") && acceptversion.empty()){
        //The requested version number is not provided
        e << "This expected parameter value is missing in the request";
        return http_fget_error(soap, "MissingParameterValue", e.str(), "acceptversions", 400);
    }

    if (!req.compare("getcapabilities")) {
        if (acceptversion.compare("2.0.0")){
            //The requested version of WFS is not implemented
            e << "The requested version of WFS is not supported by this server";
            return http_fget_error(soap,"VersionNegotiationFailed", e.str(), "", 501);
        }
        return fgetcapabilities(soap);
    }
