...
SOAP_SOURCE_STAMP("@(#) optC.c ver 2.8.92 2023-11-19 13:41:38 GMT")
...
{	(void)type;
	...
	case SOAP_TYPE_t__Option:
		return soap_in_t__Option(soap, tag, NULL, "t:Option");
	...
	*type = 0;
	...
	{	const char *t = soap->type;
		...
		{	*type = SOAP_TYPE_t__Option;
			return soap_in_t__Option(soap, tag, NULL, NULL);
		}
...
{	(void)tag;
	...
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "soap_putelement '%s' failed for type %d in webserver/optC.c\n", tag ? tag : "", type));
...
{
	struct t__Option *p;
	struct t__Option *a = (struct t__Option*)soap_malloc((soap), (n = (n < 0 ? 1 : n)) * sizeof(struct t__Option));
	for (p = a; p && n--; p++)
		soap_default_t__Option(soap, p);
	return a;
}
